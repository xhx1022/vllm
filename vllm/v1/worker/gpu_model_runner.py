...
class AsyncGPUModelRunnerOutput(AsyncModelRunnerOutput):
    ...
    def get_output(self) -> ModelRunnerOutput:
        ...
        if self._routed_experts_writer is not None:
            # The copy event above makes routing_data ready for CPU store.
            # ModelRunnerOutput is the scheduler's barrier for these slots.
            assert self._routed_experts_cpu is not None
            re_lists = self._routed_experts_cpu.tolists()
            self._routed_experts_writer.store_batch(
...
class GPUModelRunner(
    ...
    def sample_tokens(
        ...
        with record_function_or_nullcontext(
            ...
            # Clone before the async copy stream; both sources are reused next step.
    ...
    def init_routed_experts_capturer(self):
        ...
        attn_gid = require_full_attention_gid(self.kv_cache_config)
        self.routed_experts_capturer = RoutedExpertsCapturer(
            max_num_batched_tokens=self.scheduler_config.max_num_batched_tokens,
            vllm_config=self.vllm_config,
        )
        self.routed_experts_attn_gid = attn_gid
        self._bind_routed_experts_capturer(self.routed_experts_capturer)

        # Pinned CPU buffer for sync-path routing D2H.
        ...
        # block_table.slot_mapping uses int64.
        ...
        # Only the scheduler-visible rank writes the shared routing slot buffer.
        pc = self.parallel_config
        output_rank = routed_experts_output_rank()
